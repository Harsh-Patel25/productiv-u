...
export function generateCSSCustomProperties() {
  ...
  Object.entries(designTokens.typography.fontSize).forEach(([key, value]) => {
    // Handle font size values which can be arrays [size, lineHeight] or just strings
    if (Array.isArray(value)) {
      cssVariables[`--font-size-${key}`] = String(value[0]);
      if (typeof value[1] === 'object' && value[1].lineHeight) {
        cssVariables[`--line-height-${key}`] = String(value[1].lineHeight);
      }
    } else {
      cssVariables[`--font-size-${key}`] = String(value);
    }
  });
...
export const themeConfig = {
  light: {
    ...
    mutedForeground: 'var(--color-secondary-500)',
    border: 'var(--color-secondary-200)',
    input: 'var(--color-secondary-50)',
    card: 'var(--color-secondary-0, #ffffff)',
    cardForeground: 'var(--color-secondary-900)',
    primary: 'var(--color-primary-600)',
    primaryForeground: 'var(--color-secondary-50)',
  },
  ...
  dark: {
    ...
    mutedForeground: 'var(--color-secondary-400)',
    border: 'var(--color-secondary-700)',
    input: 'var(--color-secondary-800)',
    card: 'var(--color-secondary-800)',
    cardForeground: 'var(--color-secondary-50)',
    primary: 'var(--color-primary-500)',
    primaryForeground: 'var(--color-secondary-900)',
  },
};
